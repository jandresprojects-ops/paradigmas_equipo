...
def calcular_promedio(calificaciones: list) -> float:
    """Función pura para calcular el promedio."""
    if not calificaciones:
        return 0.0
    return sum(calificaciones) / len(calificaciones)

def crear_estudiante(nombre: str, edad: int, calificaciones: list) -> dict:
    """Retorna un nuevo diccionario inmutable conceptualmente."""
    return {
        'nombre': nombre.strip(),
        'edad': edad,
...
def agregar_estudiante(db_actual: tuple, nuevo_estudiante: dict) -> tuple:
    """INMUTABILIDAD: Retorna una nueva tupla combinada sin modificar la tupla original."""
    return db_actual + (nuevo_estudiante,)

def buscar_estudiantes(db_actual: tuple, nombre: str) -> list:
    """USO DE FILTER Y LAMBDA: Retorna TODOS los registros filtrados que coincidan."""
...
def calcular_promedio_general(db_actual: tuple) -> float:
    """USO DE MAP Y REDUCE: Procesa datos agregados funcionalmente."""
    if not db_actual:
        return 0.0
...
def dar_formato_estudiante(est: dict) -> str:
    """Transforma un objeto de datos a texto sin efectos secundarios."""
    return f"{est['nombre']} | Edad: {est['edad']} | Promedio: {est['promedio']:.2f}"
...
def menu_principal():
    ...
    base_datos = agregar_estudiante(base_datos, crear_estudiante("Ana García", 20, [85, 90, 88, 92]))
    base_datos = agregar_estudiante(base_datos, crear_estudiante("Ana López", 22, [90, 91, 89, 94]))
    base_datos = agregar_estudiante(base_datos, crear_estudiante("Carlos López", 19, [78, 82, 85, 80]))
    base_datos = agregar_estudiante(base_datos, crear_estudiante("María Rodríguez", 21, [95, 98, 93, 97]))
    ...
    while True:
        ...
        if opcion == '1':
            ...
            nuevo = crear_estudiante(nombre, edad, califs)
